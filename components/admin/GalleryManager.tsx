...
import { Edit, Trash2, Plus, X, Instagram, Image as ImageIcon } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-light text-white uppercase tracking-tight">Galería de Instagram</h2>
        <button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 border border-zinc-800 border-dashed rounded-lg bg-zinc-900/30">
          ...
          <p className="text-zinc-500 text-lg font-light">Aún no hay publicaciones</p>
          <p className="text-zinc-600 text-sm mt-2">Comienza creando una nueva publicación para la galería</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((post) => (
            <div key={post.id} className="bg-zinc-900 border border-zinc-800 group relative">
              ...
              <div className="p-4">
                ...
                {post.description && (
                  <p className="text-zinc-400 text-sm truncate mt-1">{post.description}</p>
                )}
                {post.instagram_url && (
                  <a 
      ...
      {(isCreating || editingPost) && (
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-zinc-400 text-xs sm:text-sm mb-2">Título</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-zinc-400 text-xs sm:text-sm mb-2">Imagen</label>
            ...
            <div>
              <label className="block text-zinc-400 text-xs sm:text-sm mb-2">Descripción (Opcional)</label>
              <textarea
            ...
            <div>
              <label className="block text-zinc-400 text-xs sm:text-sm mb-2">Link de Instagram (Opcional)</label>
      ...
      {deletingPostId && (
        ...
        >
          <div className="space-y-4">
            <p className="text-zinc-300">¿Estás seguro de que deseas eliminar esta publicación? Esta acción no se puede deshacer.</p>
